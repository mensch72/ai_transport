...
# Set up logging
logging.basicConfig(
    level=logging.DEBUG,    # 开发期 INFO，想更细可改 DEBUG
    format="%(asctime)s | %(levelname)s | %(message)s",
    filename="complex_heuristic_run.log",
    filemode="w",  # 每次运行覆盖
)
    ...
    def get_action(self, observation, action_space_size):
        ...
        if step_type == 'routing':
            ...
            if action_space_size > 1:
                ...
                #TODO: could be improved to pick best edge toward destination 选择outgoing edge所连的节点到目的地最短的一个
                best_action = 0
                best_dist = float("inf")
...
def main():
    ...
    # Log initial state
    logging.debug("=== INITIAL STATE (after manual setup) ===")
    logging.debug("agents=%s", env.agents)
    logging.debug("human_agents=%s", env.human_agents)
    logging.debug("vehicle_agents=%s", env.vehicle_agents)
    logging.debug("agent_positions=\n%s", pformat(dict(env.agent_positions)))
    logging.debug("human_aboard=\n%s", pformat(dict(env.human_aboard)))
    logging.debug("vehicle_destinations=\n%s", pformat(dict(env.vehicle_destinations)))
    logging.debug("step_type=%s", getattr(env, "step_type", None))
    ...
    for cycle in range(80):  # More cycles for complex scenario
        ...
        #LOGGING
        logging.debug("----- CYCLE %d START | step_type=%s -----", cycle, current_step)
        logging.debug("positions=\n%s", pformat(dict(env.agent_positions)))
        logging.debug("aboard=\n%s", pformat(dict(env.human_aboard)))
        logging.debug("vehicle_destinations=\n%s", pformat(dict(env.vehicle_destinations)))
        ...
        #LOGGING
        logging.debug("STEP RETURN | cycle=%d", cycle)
        logging.debug("actions=\n%s", pformat(actions))
        logging.debug("rewards=\n%s", pformat(rewards))
        logging.debug("terms=\n%s", pformat(terms))
        logging.debug("truncs=\n%s", pformat(truncs))
